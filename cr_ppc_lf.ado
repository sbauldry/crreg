  ...
  *** likelihood function for logit link
  if ( "$Link" == "logit" ) {
	...
	* build equations for middle values of Y
	...
	if ( $nCat > 3 ) {
	  forval k = 2/$nCatm1 {
	    local meqn_a `" ln(1 - invlogit(-`xb_c' - `xb_f1' - `xb_p')) + "'
        local meqn_c `" ln(invlogit(-`xb_c' - `xb_f`k'' - `xb_p'*`phi`k'')) "'
    
	    local meqn_b ""
	    local m = `k' - 1
        forval n = 2/`m' {
          local meqn_b `" `meqn_b' ln(1 - invlogit(-`xb_c' - `xb_f`n'' - `xb_p'*`phi`n'')) + "'
        }
  ...
  if ( "$Link" == "probit" ) {
	...
	* build equations for middle values of Y
	...
	if ( $nCat > 3 ) {
	  forval k = 2/$nCatm1 {
	    local meqn_a `" ln(1 - normal(-`xb_c' - `xb_f1' - `xb_p')) + "'
        local meqn_c `" ln(normal(-`xb_c' - `xb_f`k'' - `xb_p'*`phi`k'')) "'
    
	    local meqn_b ""
	    local m = `k' - 1
        forval n = 2/`m' {
          local meqn_b `" `meqn_b' ln(1 - normal(-`xb_c' - `xb_f`n'' - `xb_p'*`phi`n'')) + "'
        }
